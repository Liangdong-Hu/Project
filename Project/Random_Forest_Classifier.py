...
RandomForest = RandomForestClassifier(n_estimators=10, max_depth=None, min_samples_split=3, random_state=0)

print('RandomForest score: %f' % RandomForest.fit(X_train, y_train).score(X_test, y_test))
...
print('RandomForest ROC auc: %f' % auc(fpr, tpr))
print('RandomForest PR auc: %f' % auc(recall, precision))
print('RandomForest PR ap: %f' % metrics.average_precision_score(y_test, y_score[:, 1]))
